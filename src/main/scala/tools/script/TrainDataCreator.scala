...
object TrainDataCreator extends App {
  ...
  lazy val articles = sc.parallelize(data, Config.numberOfSparkTasks)


  def extractFeatureVectors(articles: RDD[LocationArticle]) = {
  ...
  val trainData = extractFeatureVectors(articles)
